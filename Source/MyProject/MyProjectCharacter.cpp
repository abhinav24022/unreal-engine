...
void AMyProjectCharacter::NotifyServer()
{
	...
	if (GameInstance)
	{
		if (GameInstance->WebSocket->IsConnected())
		{
			...
			int32 val = GameInstance->temp;
			//int32 val = ThePC->loot;
			FString w;
			
			if (val == 3) {
			w = "paracetamol is prescribed";
			}
			else if (val == 4) {
				w = "Crocin is prescribed";
			}
			else if (val == 5) {
				w = "go for a check up and take dolo-650";
			}
			/*if (val == 3) {
				w = "paracetamol is prescribed";
			}
			...
			FString id = GameInstance->id;
			TSharedPtr<FJsonObject> JsonObject = MakeShareable(new FJsonObject());
			//JsonObject->SetStringField("action","onMessage");
			JsonObject->SetStringField("message", w);
			JsonObject->SetStringField("id", id);
			//JsonObject->SetStringField("prescription",w);
			FString OutputString;
			TSharedRef< TJsonWriter<> > Writer = TJsonWriterFactory<>::Create(&OutputString);
			FJsonSerializer::Serialize(JsonObject.ToSharedRef(), Writer);
			GameInstance->WebSocket->Send(OutputString);
			//GameInstance->WebSocket->Send("{\"id\":\"101\",\"prescription\":\"dolo65\"}");
			//GameInstance->WebSocket->Send("{\"message\":\"hello\",\"id\":\"\" }");
...
void AMyProjectCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("NotifyServer", IE_Pressed, this, &AMyProjectCharacter::NotifyServer);
}
